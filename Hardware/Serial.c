...
uint8_t RxLen=0;
char Serial_RxPacket[100]={0};				//定义接收数据包数组，数据包格式"@MSG\r\n"
...
void Uart1_SendCharArray(char *Array, uint16_t Length)
{
	uint16_t i;
	for (i = 0; i < Length; i ++)		//遍历数组
	{
		Uart1_SendByte(Array[i]);		//依次调用Uart1_SendByte发送每个字节数据
	}
}
...
/**
  ...
  * 返 回 值：无
  */
void Uart3_SendCharArray(char *Array, uint16_t Length)
{
	uint16_t i;
	RxLen = 0;   //收到的信息长度清零
	for (i = 0; i < Length; i ++)		//遍历数组
	{
		Uart3_SendByte(Array[i]);		//依次调用Uart3_SendByte发送每个字节数据
	}
}
...
void Uart3_SendString(char *String)
{
	...
	for (i = 0; String[i] != '\0'; i ++)//遍历字符数组（字符串），遇到字符串结束标志位后停止
	{
		Uart3_SendByte(String[i]);		//依次调用Uart3_SendByte发送每个字节数据
	}
}
...
void USART3_IRQHandler(void)
{
	...
	{
		Serial_RxPacket[RxLen]=USART_ReceiveData(USART3);       //读取数据寄存器，存放在接收的数据变量
		RxLen++;
